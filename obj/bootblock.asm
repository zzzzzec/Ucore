...
protcseg:
    ...
    # 这里不可以直接把立即数放入段寄存器中，因此需要一个寄存器作为中转
...
bootmain(void) {
    ...
    7d0d:	31 c9                	xor    %ecx,%ecx
    7d0f:	ba 00 10 00 00       	mov    $0x1000,%edx
...
bootmain(void) {
    ...
    7d1c:	53                   	push   %ebx
    readseg((uintptr_t)ELFHDR, SECTSIZE * 8, 0);
    7d1d:	e8 50 ff ff ff       	call   7c72 <readseg>
 
    // is this a valid ELF?
    if (ELFHDR->e_magic != ELF_MAGIC) {
    7d22:	81 3d 00 00 01 00 7f 	cmpl   $0x464c457f,0x10000
    7d29:	45 4c 46 
    7d2c:	75 3f                	jne    7d6d <bootmain+0x61>
        goto bad;
    } 
    struct proghdr *ph, *eph;
    // load each program segment (ignores ph flags)
    ph = (struct proghdr *)((uintptr_t)ELFHDR + ELFHDR->e_phoff); 
    7d2e:	a1 1c 00 01 00       	mov    0x1001c,%eax
    //ph = elf文件的程序头（program header table）
    //这里读入了整个elf文件中的 segment
    eph = ph + ELFHDR->e_phnum;
    7d33:	0f b7 35 2c 00 01 00 	movzwl 0x1002c,%esi
    ph = (struct proghdr *)((uintptr_t)ELFHDR + ELFHDR->e_phoff); 
    7d3a:	8d 98 00 00 01 00    	lea    0x10000(%eax),%ebx
    eph = ph + ELFHDR->e_phnum;
    7d40:	c1 e6 05             	shl    $0x5,%esi
    7d43:	01 de                	add    %ebx,%esi
    for (; ph < eph; ph ++) {
    7d45:	39 f3                	cmp    %esi,%ebx
    7d47:	73 18                	jae    7d61 <bootmain+0x55>
    ...
    7d4f:	83 c3 20             	add    $0x20,%ebx
        readseg(ph->p_va & 0xFFFFFF, ph->p_memsz, ph->p_offset);
    7d52:	8b 53 f4             	mov    -0xc(%ebx),%edx
    7d55:	25 ff ff ff 00       	and    $0xffffff,%eax
    7d5a:	e8 13 ff ff ff       	call   7c72 <readseg>
    7d5f:	eb e4                	jmp    7d45 <bootmain+0x39>
    }
    // call the entry point from the ELF header
    // note: does not return
    ((void (*)(void))(ELFHDR->e_entry & 0xFFFFFF))();
    7d61:	a1 18 00 01 00       	mov    0x10018,%eax
    7d66:	25 ff ff ff 00       	and    $0xffffff,%eax
    7d6b:	ff d0                	call   *%eax
}
...
outw(uint16_t port, uint16_t data)
{
    ...
    7d6d:	ba 00 8a ff ff       	mov    $0xffff8a00,%edx
    7d72:	89 d0                	mov    %edx,%eax
    7d74:	66 ef                	out    %ax,(%dx)
    7d76:	b8 00 8e ff ff       	mov    $0xffff8e00,%eax
    7d7b:	66 ef                	out    %ax,(%dx)
    7d7d:	eb fe                	jmp    7d7d <bootmain+0x71>
